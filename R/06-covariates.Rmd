...
rem_pt <- pt_stg %>%
  ...
  dplyr::filter(grepl("Remdesivir", COVID_tx)) %>%
  pull(deid_enc_id) %>%
  unique()
...
cort_pt <- pt_stg %>%
  dplyr::filter(model == "base_model") %>%
  mutate(COVID_tx = as.character(COVID_tx)) %>%
  dplyr::filter(grepl("Corticosteroid", COVID_tx)) %>%
  pull(deid_enc_id) %>%
  unique()

cov_tx_plot_dat <- pt_stg %>%
  dplyr::filter(model == "base_model") %>%
  # dplyr::filter(!deid_enc_id %in% rem_pt) %>%
  ...
  mutate(COVID_tx = case_when(
    COVID_tx %in% c("No Treatment", "Monoclonal Antibody", "Remdesivir only") ~ "No Corticosteroids",
    COVID_tx %in% c("Remdesivir + Corticosteroid") ~ "Corticosteroids",
    TRUE ~ as.character(COVID_tx)
  ) %>% factor()) %>%
  mutate(COVID_tx = relevel(COVID_tx, ref = "No Corticosteroids"))
...
temp <- cov_tx_plot_dat %>%
  dplyr::select(deid_enc_id, DaysSinceEntry, COVID_tx)
...
cov_tx_plot_dat %>% 
  ...
  mutate(COVID_tx = as.character(COVID_tx)) %>%
  dplyr::filter(COVID_tx != "No Corticosteroids") %>%
  distinct() %>%
  group_by(COVID_tx) %>%
  dplyr::summarize(n = n())
...
p <- ggplot(cov_tx_plot_dat, aes(x = DaysSinceEntry, y = deid_enc_id, 
  ...
  theme_minimal() +
  scale_fill_manual(breaks = c("No Corticosteroids", "Corticosteroids"),
                    values = c(pal)) +
  xlim(0, 21) +
  theme(axis.text.y = element_blank(),
...
cov_tx_combined <- cov_tx_dist + cov_tx_hr + plot_layout(widths = c(2.5, 1))
