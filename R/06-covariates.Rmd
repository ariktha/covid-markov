...
temp_id <- tab_one_enc_dat %>% group_by(deid_enc_id) %>% summarise(n = n()) %>% filter(n > 1) %>% pull(deid_enc_id)
temp_row <- tab_one_enc_dat %>% filter(deid_enc_id %in% temp_id) %>% mutate(vax = factor(1)) %>% distinct()

tab_one_enc_dat <- tab_one_enc_dat %>%
  filter(!deid_enc_id %in% temp_id) %>%
  bind_rows(temp_row) %>%
  distinct()
...
age_hr <- ggplot(subset(age_hazards_plot, constraint_type == "transition_specific"), 
       aes(color = covariate_term, shape = covariate_term)) +
  ...
  scale_shape_manual(breaks = age_quintile_colors$model_term,
                     values = c(18, rep(20, 4)),
                     labels = age_quintile_colors$model_label
                     ) +
  labs(x = "Hazard Ratio (log scale)", y = "Transition", color = NULL, shape = NULL) +
  theme_minimal() +
...
age_support_dat <- pt_stg %>%
  dplyr::filter(model == "base_model") %>%
  dplyr::select(deid_enc_id, DaysSinceEntry, age, state) %>%
  distinct() %>%
  arrange(deid_enc_id, DaysSinceEntry) %>%
  group_by(deid_enc_id) %>%
  mutate(
    from_state = lag(state),
    to_state   = state
  ) %>%
  ungroup() %>%
  dplyr::filter(!is.na(from_state)) %>%   # removes first row per person
  dplyr::select(deid_enc_id, age, DaysSinceEntry, from_state, to_state) %>%
...
age_limits <- age_support_dat %>%
  dplyr::select(age, transition) %>%
  group_by(transition) %>%
  dplyr::summarize(min_age = min(age, na.rm = TRUE), 
                   max_age = max(age, na.rm = TRUE), .groups = "drop")

age_hazards_long_plot <- age_hazards_long %>%
  left_join(age_limits, by = "transition") %>%
  dplyr::filter(covariate_value >= min_age & covariate_value <= max_age)

age_hr_sp <- ggplot(age_hazards_long_plot, aes(x = covariate_value, y = hr, fill = model_type)) +
  geom_line(aes(color = model_type)) +
  geom_ribbon(aes(ymin = hr_lower, ymax = hr_upper), alpha = 0.2, color = NA) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  geom_rug(data = age_support_dat, aes(x = age), 
...
ggsave(here("figs", "age_combined1.png"), age_combined1, width = 8, height = 8)
...
age_combined2 <- age_hist + age_logl_nc + age_hr_sp + age_hr +
  plot_layout(heights = c(2, 1, 4), design = design2) +
  plot_annotation(tag_levels = 'A')

age_combined2

ggsave(here("figs", "age_combined2.png"), age_combined2, width = 9, height = 8)
...
sort_list <- cov_tx_plot_dat %>%
  dplyr::select(deid_enc_id, DaysSinceEntry) %>%
  group_by(deid_enc_id) %>%
  dplyr::summarize(total_days = max(DaysSinceEntry, na.rm = TRUE), .groups = "drop") %>%
  arrange(total_days) %>%
  pull(deid_enc_id)


cov_tx_plot_dat <- cov_tx_plot_dat %>%
  mutate(deid_enc_id = factor(deid_enc_id, levels = sort_list)) %>%
...
pal = viridis(4, option = "plasma", direction = 1)
...
p <- ggplot(cov_tx_plot_dat, aes(x = DaysSinceEntry, y = deid_enc_id, 
  ...
  theme_minimal() +
  scale_fill_manual(values = pal, breaks = c("No Treatment", 
                                      "Remdesivir Only", 
                                      "Corticosteroids Only", 
                                      "Both")) +
  xlim(0, 21) +
...
cov_tx_hrs <- tidy_msm_hazards(cov_tx_mod)
...
cov_tx_hr_plot <- cov_tx_hrs %>%
  ...
  mutate(COVID_tx = str_remove(covariate_term, "^COVID_tx")) %>%
  bind_rows({
    x <- .[NA, ]
    x$COVID_tx <- "No Treatment"
    x$transition <- "M - S"
    x
  }) %>%
  mutate(COVID_tx = factor(COVID_tx, 
...
cov_tx_hr <- ggplot(cov_tx_hr_plot, aes(y = transition, x = hazard_ratio, color = COVID_tx)) +
  ...
  theme_minimal() +
  xlim(c(x_min, x_max)) +
  scale_color_manual(values = pal, breaks = c("No Treatment", 
                                      "Remdesivir Only", 
                                      "Corticosteroids Only", 
                                      "Both")) +
  scale_alpha_manual(values = c(`TRUE` = 1, `FALSE` = 0), guide = "none") +
...
cov_tx_combined <- cov_tx_dist + cov_tx_hr + 
  # plot_annotation(tag_levels = 'A') +
...
ggsave(here("figs", "cov_tx_combined.png"), cov_tx_combined, width = 8, height = 5)
