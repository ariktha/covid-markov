...
pmat_plot <- ggplot(pmat_long, aes(x = time, y = p_estimate, color = trend)) +
  ...
  scale_color_manual(values = trend_colors, name = "Trend") +
  ...
  guides(color = guide_legend(override.aes = list(size = 4, shape = 15, alpha = 1), nrow = 2, byrow = TRUE))
...
st_plot + 
  (soj_plot / prev_plot + plot_layout(guides = "collect")) +
  (trans_intens_plot / pmat_plot + plot_layout(guides = "collect") &
