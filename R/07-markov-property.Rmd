...
y_scales <- list(
  `Number of Parameters` = scale_y_continuous(limits = c(5, 26), n.breaks = 4),
  `Log-Likelihood` = scale_y_continuous(limits = c(-4550, -4400), n.breaks = 4),
  `AIC` = scale_y_continuous(limits = c(8800, 9100), n.breaks = 4),
  `BIC` = scale_y_continuous(limits = c(9000, 9150), n.breaks = 4)
)
...
comp_plot + haz_plot + table_plot + haz_spline_plot +
  plot_layout(design = design,
              guides = "collect",
              heights = c(0.5, 0.5, 1.5),
              widths = c(2.5, 3)) &
  theme(legend.position = "bottom")

ggsave(
  filename = here("figs", "markov-property.png"),
  width = 8,
  height = 7,
  units = "in",
  dpi = 300
