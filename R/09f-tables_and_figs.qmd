...
models_for_markov_comp <- readRDS(here("data", "temp", "markov_model_comp.rds"))
...
![State space diagram](figs/state-space.png)
...
models_summary <- base_models_comp[["model_summary"]] %>%
  left_join(models_df, by = "model") %>%
  dplyr::select(model_name, n_params, loglik, AIC, BIC, n_obs, optimization_method, status)

kable(models_summary, 
      col.names = c("Model", "Number of parameters", "Log-likelihood", 
                    "AIC", "BIC", "Number of observations", 
                    "Optimization method", "Status"),
      digits = c(0, 0, 2, 1, 1, 0, 0, 0)) %>%
  kable_styling(full_width = FALSE)

```

```{r}
#| label: model-descriptions-plot
#| echo: true

models_summary_long <- models_summary %>%
  ...
  mutate(model_name = factor(model_name, levels = models_df$model_name),
         metric = factor(metric, levels = c("n_params", "loglik", "AIC", "BIC"),
                         labels = c("Number of parameters", "Log-likelihood", "AIC", "BIC")),
         model_name_plot = case_when(
           model_name == "Base model" ~ "Base model",
           model_name == "State for history of severe" ~ "State for history\nof severe",
           model_name == "2 moderate states" ~ "2 moderate\nstates",
           model_name == "3 moderate states" ~ "3 moderate\nstates",
           model_name == "2 severe states" ~ "2 severe\nstates",
           model_name == "Simplified transitions" ~ "Simplified\ntransitions",
           TRUE ~ "NA"
         )) 

ggplot(models_summary_long, aes(x = model_name_plot, y = value)) +
  geom_point(size = 3) +
...
qmat_data <- add_transition_trends(qmat_data)
...
kable(qmat_data %>%
      ...
      col.names = c("Model", "Transition", "Trend", 
...
reactable(
  ...
  columns = list(
    ...
    text = colDef(
      name = "Transition intensity (per day)",
      format = colFormat(digits = 3),
      minWidth = 180
    )
  ),
  
  # Styling similar to your kable
  striped = TRUE,
  highlight = TRUE,
  bordered = TRUE,
  compact = TRUE,
...
ggplot(markov_models_summary_long, aes(x = model_name, y = value, color = model_name)) +
  geom_point(size = 3) +
