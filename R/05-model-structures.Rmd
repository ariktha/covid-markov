...
facet_limits <- data.frame(
  metric = factor(metric_order,
                  levels = metric_order),
  ymin = c(0.07, 0.18, 0.5, 0.8, NA, NA, 5, 5),
  ymax = c(0.09, 0.28, 0.8, 0.9, NA, NA, 10, 8)
)
...
ggplot(subset(pred_err_summ, model_name != "Base model" & model_name != "History of severe"))  +
  ...
  scale_x_discrete(limits = c("3 moderate states", 
                              "2 moderate states", 
                              "2 severe states", 
                              # "History of severe", 
