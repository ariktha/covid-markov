...
dem <- demographics_and_event_table %>%
  ...
  mutate(
    ...
    ethnicity = case_when(
      ethnicity %in%
        c("Unknown", "Declined", "Unknown/Declined", "NULL") ~ "Unknown",
      TRUE ~ ethnicity
    ),
    smoking = case_when(
      smoking %in%
    ...
    insurance_type = case_when(
      insurance_type == "01 - Medicare" ~ "Medicare",
...
comorb <- comorbidity::comorbidity(
  all_encounter_diagnoses,
  id = "deid_enc_id",
  code = "current_icd10_list",
...
all_copd <- all_encounter_diagnoses %>%
  ...
  summarise(copd_sum = sum(copd)) %>%
  mutate(copd = ifelse(copd_sum, 1, 0)) %>%
  dplyr::select(deid_enc_id, copd)
...
dm_cov_stg_summ <- dm_covid_stg %>%
  ...
  rename(DaysInSevere = DaysInState) %>%
  dplyr::select(-state) %>%
  mutate(EverSevere = TRUE)

dm_cov_summ <- dm_cov_summ %>%
