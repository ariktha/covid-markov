...
fit_msm_models_old <- function(patient_data, 
                           crude_rates, 
                           covariates = NULL, 
                           spline_vars = NULL, 
                           spline_df = 3,
...
#' @return Nested list of fitted models: fitted_models[[model_structure]][[formula]]
    ...
    for (constraint_name in names(constraint_configs)) {
      ...
      if (constraint_name == "constant") {
        ...
        models_result <- fit_msm_models(
          ...
          # No nest_name to avoid extra nesting layer
        )
        ...
        if (!is.null(models_result)) {
          for (model_structure in names(models_result)) {
            # Create new model structure name that includes constraint info
            new_model_name <- paste(model_structure, constraint_name, sep = "_")
            transition_models[[new_model_name]] <- models_result[[model_structure]]
          }
        }
        ...
        # Add constraint metadata and rename model structures
        if (!is.null(models_result)) {
          for (model_structure in names(models_result)) {
            new_model_name <- paste(model_structure, constraint_name, sep = "_")
            transition_models[[new_model_name]] <- models_result[[model_structure]]
            
            # Add constraint metadata to each formula within this model structure
            for (formula_name in names(models_result[[model_structure]])) {
              if (is.list(models_result[[model_structure]][[formula_name]])) {
                transition_models[[new_model_name]][[formula_name]]$constraint_type <- "by_trend"
              }
            }
          }
        }
        ...
        if (!is.null(models_result)) {
          for (model_structure in names(models_result)) {
            ...
            for (formula_name in names(models_result[[model_structure]])) {
              if (is.list(models_result[[model_structure]][[formula_name]])) {
                transition_models[[new_model_name]][[formula_name]]$constraint_type <- "by_transition"
              }
            }
          }
        }
