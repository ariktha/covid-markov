...
```{r base-model-overview}
base_model_summary <- same_structure_comparisons$base_models %>%
  select(model, converged, AIC, BIC, n_params, delta_AIC) %>%
  arrange(AIC)
...
base_model_summary %>%
  ...
  kable(caption = "Base Model Comparison Summary") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) %>%
  row_spec(which.min(base_model_summary$AIC), bold = TRUE, color = "white", background = "darkgreen")
```

## Structural Model Comparison (DRAIC/DRLCV)
...
significant_covariates <- covariate_stats %>%
  ...
  arrange(lrt_pval) %>%
  slice_head(n = 20)  # Top 20 most significant
...
continuous_comparison <- covariate_stats %>%
  ...
  select(covariate, type, model, AIC, BIC, lrt_pval) %>%
  pivot_wider(names_from = type, values_from = c(AIC, BIC, lrt_pval), names_sep = "_") %>%
  mutate(
    delta_AIC_spline_vs_linear = AIC_spline - AIC_linear,
    delta_BIC_spline_vs_linear = BIC_spline - BIC_linear,
    prefer_spline_AIC = delta_AIC_spline_vs_linear < -2,
    prefer_spline_BIC = delta_BIC_spline_vs_linear < -2
  )

continuous_comparison %>%
  select(covariate, model, delta_AIC_spline_vs_linear, delta_BIC_spline_vs_linear, 
         prefer_spline_AIC, prefer_spline_BIC) %>%
