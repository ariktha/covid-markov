...
all_encounter_diagnoses <- all_encounter_diagnoses_raw %>%
  ...
  mutate(across(
    c(present_on_admit, severity_of_dx, co_morbidity, cc_or_mcc),
    as.factor
  ))
...
dm <- demographics_and_event_table %>%
  ...
  group_by(deid_enc_id) %>%
  mutate(
    DaysSinceEntry = difftime(date, min(date), units = "days"),
    DaysSincePos = difftime(date, covid_pos_date, units = "days")
  ) %>%
  ungroup()
...
SF.tab <- fio2.tab %>%
  left_join(
    spo2.tab,
    by = c("deid_enc_id", "recorded_time"),
    suffix = c(".fi", ".sp")
  ) %>%
  mutate(SF = (meas_val_num.sp / meas_val_num.fi * 100)) %>%
  dplyr::select(deid_enc_id, recorded_time, date.fi, SF) %>%
  rename(date = date.fi)
...
SF200.tab <- SF.tab %>%
  mutate(sf_lt_200_yn = case_when(SF < 200 ~ 1, is.na(SF) ~ 0, TRUE ~ 0)) %>%
  group_by(deid_enc_id, date) %>%
...
device.tab <- fs %>%
  ...
  mutate(
    NOdev_yn = ifelse(meas_val_chr %in% no_dev_names, 1, 0),
...
O2.tab <- fs %>%
  mutate(
    O2 = ifelse(flo_meas_name %in% O2_names & meas_val_num > 0, 1, 0),
    LowO2 = ifelse(
      flo_meas_name %in% O2_names & meas_val_num > 0 & meas_val_num <= 12,
      1,
      0
    ),
  ...
  distinct(deid_enc_id, date, O2, LowO2, HighO2)
